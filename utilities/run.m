...
function run(trialID, training_state, training_dataset_path, LearnedModelPath)
clc;
close all;
global rootPath saveDir
global TRAINING_SUCCESS_FAILURE TRAINING_PLOT_ON STATE TRUNCATION_STATES TRUNCATION_COMPONENTS Niter
...
plot(sensor');
grid on;
title(strcat(STATE(training_state),': ','Orginal signal'));
...
grid on;
title(strcat(STATE(training_state),': ','After data preprocessing: mean = 0, covariance = unit matrix'));
...
%% Postprocessing
cd (LearnedModelPath);
LearnedModelFile = strcat(TRAINING_SUCCESS_FAILURE,'_',char(STATE(training_state)),'_',num2str(settings.Niter),'_',num2str(trialID),'.mat');
