...
function train_task(trialID, training_dataset_path, LearnedModelPath)
clc;
close all;
global rootPath saveDir METHOD
global TRAINING_SIM_REAL TRAINING_SUCCESS_FAILURE TRAINING_PLOT_ON TRUNCATION_STATES TRUNCATION_COMPONENTS Niter
[sensor, R_State, foldname]= load_one_trial(trialID, training_dataset_path);

figure;
subplot(2,1,1);
plot(sensor');
grid on;
title(strcat(TRAINING_SIM_REAL,'\_',TRAINING_SUCCESS_FAILURE,'\_',foldname,': ','Orginal signal'));
...
switch caseNumber
    case 1
        ...
        Kz = TRUNCATION_STATES;  % truncation level of the DP prior on HMM transition distributions pi_k
        Ks = TRUNCATION_COMPONENTS;  % truncation level of the DPMM on emission distributions pi_s       
    case 2
        obsModelType = 'Gaussian';
...
[sensor, meanSigma] = data_preprocessing(sensor);   
...
subplot(2,1,2);
plot(data_struct.obs');
grid on;
title(strcat(TRAINING_SIM_REAL,'\_',TRAINING_SUCCESS_FAILURE,'\_',foldname,': ','After data preprocessing: mean = 0, covariance = unit matrix'));
...
switch obsModelType
   ...
   case 'SLDS'
        model.obsModel.params.nu = d + 2;                                            %must be > d + 1: how strongly we believe the covariance(shape) in this prior
end                                          %must be > d + 1: how strongly we believe the covariance(shape) in this prior
...
[stateSeq,                ...
    ...
    data_struct] =  HDPHMMDPinference(data_struct, model, settings, 'TASK',trialID);  
%% Postprocessing
cd (LearnedModelPath);
LearnedModelFile = strcat(TRAINING_SIM_REAL, '_', TRAINING_SUCCESS_FAILURE,'_',foldname,'.mat');
