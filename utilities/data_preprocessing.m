...
%vY = var(diff(sensor'));% default  :    varance for each dim
vY = var((sensor'));     %Hongmin Wu:  varance for each dim

for j=1:length(vY)
    sensor(j,:) = sensor(j,:) ./ sqrt(vY(j));
end 
%meanSigma = 0.75 * cov(diff(sensor'));  %If bad segmentation, try values between 0.75 and 5.0 
meanSigma = 0.75 * cov(sensor');  %If bad segmentation, try values between 0.75 and 5.0 
