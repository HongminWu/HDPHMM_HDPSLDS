...
function [sensor, meanSigma] = data_preprocessing(sensor)
...
vY = var(diff(sensor'));% varance for each dim
%vY = var((sensor'));% varance for each dim
%vY(vY < 0.0001) = 0.0001; %so as to avoid the denominator is 0

for j=1:length(vY)
    sensor(j,:) = sensor(j,:) ./ sqrt(vY(j));
end 
meanSigma = 2.0 * cov(diff(sensor'));  %If bad segmentation, try values between 0.75 and 5.0 
%meanSigma = 1.0 * cov((sensor'));  %If bad segmentation, try values between 0.75 and 5.0 
