...
switch caseNumber
    ...
    case 2
        obsModelType = 'Gaussian';
        priorType = 'NIW';  %both the mean and the covariance are unknown
...
settings.plotEvery = 20; %plot frequency
...
model.HMMmodel.params.c=100;  % self-transition proportion parameter. Beta(c, d) rho0 = c/(c+d) (In paper: rho = kappa/(kappa + alpha))
...
%% --display-----   
fHandle = figure; plot(data_struct.obs'); 
hold on; 
plot(data_struct.true_labels,'r','LineWidth',2);
...
for seq=1
    ...
    for t=trial_vec
        ...
        [stateSeq, theta, hyperparams] =  HDPHMMDPinference(data_struct, model,settings, char('Synthetic_Data'));
...
%             for m = 1:  settings.Ks
%                 for n = 1: settings.Kz 
%                         plotGMM(theta.mu(:,n,m), inv(theta.invSigma(:,:,n,m)), [.8 0 0]);
%                         hold on;
%                 end
%             end
%         end
