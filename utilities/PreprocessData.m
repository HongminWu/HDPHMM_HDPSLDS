function data_struct = PreprocessData(model,settings,T,sensor)
...
pi_init = dist_struct.pi_init; %sum(pi_init) = 1
...
for t=1:T
    if t>1
        p_z = cumsum(pi_z(labels(t-1),:)); %cumsum(),  Cumulative sum of elements.
...
%           Y(:,t) = mu(:,labels(t),slabels(t)) + cholSigma(:,:,labels(t),slabels(t))'*randn(d,1);
           Y(:,t) = mu(:,labels(t),slabels(t)) + cholSigma(:,:,labels(t),slabels(t))'*sensor(:,t);
        end
