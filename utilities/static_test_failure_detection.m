...
function static_test_failure_detection(trialID)
...
global  TRUE_POSITIVE  FALSE_NEGATIVE FALSE_POSITIVE TRUE_NEGATIVE CONST_C
...
for nState = 1 : length(STATE) 
    ...
    stateThreshold            = state_expected_likelihood - CONST_C * sigma;
 
%     % Load Model for selected state. We will compare other observations with this.
%     model_select.OPTIMAL_MODEL = randpick(model_id);
      model_select.OPTIMAL_MODEL = trialID;
...
for nState = 1:length(STATE)
    ...
    for nModel = 1:length(trainedModel) %  the same data for different state model
        ...
        current_log =  stateLikelihood(nState,R_State(nState) + obsModel.r:R_State(nState + 1)); 
        diff = current_log(end) - thresholdcell{nState}(end);
        if  isempty(diff(find(diff < 0)))
            STATE_STATUS = [STATE_STATUS, true];
        else
            STATE_STATUS = [STATE_STATUS, false];
        end 
end
    %only for failure detection
...
if PLOT_SAVE
    ...
    saveas(gHandle_testing, strcat('FAILURE_DETECTION','_',foldname),'jpg');
end
end